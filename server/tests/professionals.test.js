...
describe('Professionals CRUD', () => {
  ...
  test('create, list, update, delete professional', async () => {
    const created = await request(app).post('/api/professionals').send({ name: 'Bob', phone: '222' }).expect(201);
    ...
    const updated = await request(app).put(`/api/professionals/${id}`).send({ phone: '333' }).expect(200);
    expect(updated.body.phone).toBe('333');
