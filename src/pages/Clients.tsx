...
export default function Clients() {
  ...
  const [form, setForm] = useState<Partial<Client>>({ name: '', email: '', phone: '' });
  const [editing, setEditing] = useState<Client | null>(null);

  const createMut = useMutation({
    mutationFn: (payload: Partial<Client>) => api.post('/clients', payload),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['clients'] }); setForm({ name: '', email: '', phone: '' }); },
  });
  const updateMut = useMutation({
    mutationFn: (payload: Partial<Client>) => api.put(`/clients/${editing!.id}`, payload),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['clients'] }); setEditing(null); setForm({ name: '', email: '', phone: '' }); },
  });
